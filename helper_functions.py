...
def format_values(console_output, num_input_blocks, num_block_sets=1):
        ...
        that the Python testing functions can work with.

        Args:
            console_output (str): The kernel prints hex dumps of each input block (unswizzled), the pdep marker stream
            block, and the output blocks (swizzled). console_output is created from that. E.g.:
        ...
        Returns:
            ...
            00 00 00 00 00 01 20 88 04 48 10 81 12 10 80 80 82 20 41 22 04 08 10 21 10 81 02 88 10 20 40 11 -> [0]:7308569839543276070860605882704647002519782667084839218520512242113021214784
            00 00 00 00 00 00 d0 44 00 00 00 00 01 00 00 00 00 00 00 01 00 00 00 00 00 00 00 00 00 00 20 00 -> [1]:210624583337114373395836055367340864637790190801098222508621955072
            00 00 00 00 00 00 00 00 02 24 08 40 88 08 40 40 41 10 20 90 02 04 08 10 88 40 81 44 08 10 00 08 -> [2]:3654284919561013452093188567956487445892550468904629417868863214089089843200
            ff ff ff ff ff ff ff ff fd db f7 bf 77 f7 bf bf be ef df 6f fd fb f7 ef 77 bf 7e bb f7 ef ff f7 -> [3]:10819726235100219966738542146045191097401082953856532763855286610790526746688
        ...
        for j in range(num_block_sets):
            ...
            lines_array = console_output.split('\n')
            for i in range(len(lines_array)):
                lines_array[i] = lines_array[i].split('=')[-1] # take everything after the '='

            # extract input, pdep, and output block data
            read_start = block_start
            read_end = block_start + num_input_blocks
            input_blocks = lines_array[read_start:read_end]
            input_blocks = [int(x, 16) for x in input_blocks] # convert from hex string to decimal int
            read_start = read_end
            read_end = read_start + 1
            pdep_ms_block = lines_array[read_start:read_end]
            pdep_ms_block = int(pdep_ms_block[0], 16)
            read_start = read_end
            read_end = read_start + num_input_blocks
            output_blocks = lines_array[read_start:read_end]
...
def compare_expected_actual(tester, block_sets, block_width=256, num_input_blocks=4):
    ...
    Args:
        ...
        block_width (int): The width of a block in the Parabix program. Used when we stitch blocks of input 
            together into a single, long input block that gets passed to the Python program (Python has unlimited
            precision ints so we do everything at once instead of breaking the input into blocks).
